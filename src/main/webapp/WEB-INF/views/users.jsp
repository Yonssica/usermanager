...
<div>
    <table class="easyui-datagrid" id="userList" title="会员列表"
           data-options="singleSelect:false,collapsible:true,pagination:true,url:'/user/list',method:'post',pageSize:5,toolbar:toolbar,pageList:[5,10,20]">
        <thead>
        <tr>
...
<div id="userAdd" class="easyui-window" title="新增会员"
     data-options="modal:true,closed:true,iconCls:'icon-save',href:'/user/page/user-add'"
     style="width:800px;height:600px;padding:10px;">
    The window add.
</div>
<div id="userEdit" class="easyui-window" title="新增会员"
     data-options="modal:true,closed:true,iconCls:'icon-edit',href:'/user/page/user-edit'"
     style="width:800px;height:600px;padding:10px;">
    The window edit.
</div>
    ...
    var toolbar = [{
        ...
        handler: function () {
            ...
            if (row == null || row.length == 0) {
                ...
                $.messager.alert('警告', '请勿选择多行!', 'warning');
                return;
            } else {
                $("#userEdit").window({
                    onLoad: function () {
                ...
                if (r) {
                    $.post("/rest/user", {'ids': ids,"_method":"DELETE"}, function (data,status,xhr) {
                        if (xhr.status == 204) {
                            ...
                            $.messager.alert('警告','删除会员失败！','warning');
                        }
                    });
                }
            });
